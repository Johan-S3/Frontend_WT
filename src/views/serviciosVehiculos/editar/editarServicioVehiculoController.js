...
export const editarServicioVehiculoController = async (parametros = null) => {
  /* ------------------ VARIABLES ------------------  */
  const { id } = parametros
  ...
  const ServicioVehiculoRec = await obtenerDatos(`serviciosVehiculos/${id}`);

  // Destructuro la respuesta para solo obtener su data.
  const { data } = ServicioVehiculoRec;

  const servicioVeheculo = data[0];

  nombre.value = servicioVeheculo.nombre_servicio;
  porcentaje.value = servicioVeheculo.porcentaje_descuento;
  ...
  formRegistro.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const respuesta = await editarDato("serviciosVehiculos", id, servicioVehiculo);
