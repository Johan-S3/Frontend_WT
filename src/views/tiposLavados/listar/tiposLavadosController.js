...
export const tiposLavadosController = async (parametros = null) => {
    /* ------------------ VARIABLES ------------------  */
...
async function cargarTiposLavados(contendor) {
    try {
        ...
        const tiposLavados = await obtenerDatos("tiposLavados");
        ...
        tiposLavados.data.forEach(async (tipoLavado) => {
            ...
            fila.setAttribute("data-id", tipoLavado.id)

            // Declaro y asigno a una variable un arreglo con las celdas de la fila
            const celdas = [celdaId, celdaNombre, celdaDescripcion, celdaTipoVeh, celdaAcciones];
            ...
            celdaId.textContent = tipoLavado.id;
            celdaNombre.textContent = tipoLavado.nombre;
            celdaDescripcion.textContent = tipoLavado.descripcion;
            celdaTipoVeh.textContent = tipoLavado.nombre_tipo;

            const permisos = getCookie("permisos", []);

            permisos.forEach(permiso => {
                if (permiso == "tipos_lavados.update") {
                    ...
                    botonEditar.setAttribute("data-id", tipoLavado.id)
                ...
                if (permiso == "tipos_lavados.delete") {
                    ...
                    botonEliminar.setAttribute("data-id", tipoLavado.id)
...
async function cargarBoton(contendor) {
    ...
    permisos.forEach(permiso => {
        if (permiso == "tipos_lavados.create") {
...
async function eliminarTipoLavado(id) {
    ...
    const peticion = await eliminarDato("tiposLavados", id);
    ...
    if (peticion.code == 200) {
        ...
        await successAlert("Tipo de lavado eliminado correctamente");
        // location.reload();
    } else {
...
async function cargarVistaCrear() {
    window.location.href = "#/tiposLavados/crear";
}