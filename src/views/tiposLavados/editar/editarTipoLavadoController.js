...
export const editarTipoLavadoController = async (parametros = null) => {
    /* ------------------ VARIABLES ------------------  */
    const { id } = parametros
    ...
    const tiposLavadosRec = await obtenerDatos(`tiposLavados/${id}`);

    // Destructuro la respuesta para solo obtener su data.
    const { data } = tiposLavadosRec;

    console.log(data);

    const tipoLavado = data[0];


    nombre.value = tipoLavado.nombre;
    descripcion.value = tipoLavado.descripcion;
    selectTipoVeh.value = tipoLavado.id_tipo_vehiculo;
    ...
    const campos = [nombre, descripcion, selectTipoVeh];
    ...
    formRegistro.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            const respuesta = await editarDato("tiposLavados", id, tipoLavado);
