...
export const editarItemLavadoController = async (parametros = null) => {
    /* ------------------ VARIABLES ------------------  */
    const { id } = parametros
    ...
    const itemsLavadosRec = await obtenerDatos(`itemsLavados/${id}`);



    // Destructuro la respuesta para solo obtener su data.
    const { data } = itemsLavadosRec;

    console.log(data);
    const itemLavado = data;


    nombre.value = itemLavado.nombre;
    descripcion.value = itemLavado.descripcion;
    valor.value = itemLavado.valor;
    selectTipoVeh.value = itemLavado.id_tipo_vehiculo;
    ...
    formRegistro.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            const respuesta = await editarDato("itemsLavados", id, itemLavado);
